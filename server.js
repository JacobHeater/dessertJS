(() => {
    ...
    defaults.map(d => app.get(d, (request, response) => response.redirect(302, "/examples/countries")));

    app.listen(port, function() {
        console.log(`Listening on port ${port}.`);
        //Open chrome in a new window pointing to the address of our application.
        cmd(`start cmd /c start chrome --app="http://localhost:${port}"`, (error, stdout, stderr) => {
            if (error) {
